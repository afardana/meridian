...
- **Gas-fee accounting (partially fixed 2026-06-22).** The per-tx fee lookup (`fetchTxFeeLamports` in dlmm.js, used by deploy/close + wallet.js swaps) previously called `getTransaction` immediately after confirmation, which usually returns null, so every tx defaulted to the 5000-lamport base fee (the tell: closed positions all showed `total_gas_sol = 0.00002` = 4×5000). Now retried (4×, 800ms), so priority fees are captured. **Still uncaptured:** the **exit-swap price impact/slippage** — `close_position` runs `recordPerformance` *before* the executor's auto-swap (base→SOL) fires, so `final_value_usd` is market-priced and the slippage on actually selling an illiquid base token (plus that swap's gas) never lands in the position's PnL. For small meme positions this can exceed the tx fees, so realized PnL is mildly optimistic. Fixing means feeding the post-close swap result back into the closed record — deferred.
