...
**Jupiter swap key:** the constant in `tools/wallet.js` was reviewed by the operator on 2026-09-26 and marked safe (no rotation, no `JUPITER_API_KEY` in .env); it is the shared rate-limited key, hence the 429-aware auto-swap retry.
...
- **Confirmation counts distinct valuations** (2026-09-25, audit 01 §8): 82% of consecutive 5 s poller ticks repeat the previous PnL reading (the valuation refreshes ~every 15 s), so "N consecutive ticks" was mostly one valuation seen N times — a single spurious reading confirmed peaks and exit signals (GO-SOL closed on a one-valuation +1.01% blip; a +223% blip fired take-profit; fake +2.7% peaks armed trailing then closed at +1%). `assessValuation(p)` (index.js) keys each tick by `pnl|active_bin|value`; `confirmPeak` and `registerExitSignal({fresh})` only advance on a distinct valuation and never fire on a stale one, and a positive jump > `pnlJumpSuspectPp` (management, 15 pp) between two valuations marks the reading `pnl_pct_suspicious` for as long as it persists (downward jumps untouched — crashes are real). `[PNL_JUMP]` log.
- **Toxic-conversion exit** (management, from 07443b1 2026-09-16; `toxicConversionEnabled`, 85% / 20 min / 1.5%) — **OFF in prod since 2026-09-25** (code default false): closes a ladder that is ≥85% converted to base within 20 minutes with fees < 1.5%, i.e. a dip ladder doing what it was placed to do; its only two fires were the operator's MET-SOL positions at 5 and 8 minutes (−0.69%, −1.01%). Shadow log `[TOXIC_SHADOW]`-style would-fire remains via the existing shadow branch. Replay before re-enabling.
- **Adopted-position profit grace** (management, 2026-09-25, operator request): `adoptedProfitGraceMinutes` (default **60**; 0 = off). For operator-created positions the bot adopts, no profit-taking rule runs for the first N minutes after `adopted_at` — trailing TP does not arm or fire, RULE_2 take-profit and the round-trip harvest are skipped (`isProfitExitSuppressed(pos, action, cfg)` in state.js, alongside the range-harvest profile). Downside rules (stop loss, crash/rug, OOR-below, low-yield with its own `adoptGraceMinutes`) keep applying. The confirmed peak keeps updating during the grace, so trailing arms against the true peak once the grace ends. Bot deploys are unaffected. Log `[ADOPT_GRACE]` once per position. Background: trailing had closed two manual GO-SOL positions at +1% within 6–11 minutes of adoption on fake peaks (audit 01 §8).
- **Pair-adaptive crash regime** (management, 2026-09-26, P(DOOM)-SOL review; `crash-regime.js`): `crashRegimeMode` (code default **shadow**, **prod enforce since 2026-09-26**; off | shadow | enforce), `crashRegimeNoiseGate` 3, `crashRegimeNoisePrior` 6, `crashRegimeK` 2.5, `crashRegimeVMin` 6 / `VMax` 20, `crashRegimeDMin` 3, `crashRegimeConfirm` 2, eligibility `crashRegimeMaxVolatility` 6 / `MinTvl` 50k / `MinTokenAgeHours` 24. Ordinary 60 s in-range dips differ 5–10× by pair type (p95: token >7 d ≈ 1 bin, <24 h ≈ 4, volatility ≥6 ≈ 7, TVL <100k ≈ 6–7), so the fixed 12 b/min crash/rug bar is sluggish on calm pairs and noise-tripping on fresh ones. On a CALM pair (profile-eligible AND the pair's own causal noise p95 ≤ gate) a sensitive rule runs beside the live detectors — peak-to-current velocity (a flat stretch no longer dilutes a sudden drop) ≥ clamp(K×noise, VMin, VMax), distance below the edge ≥ clamp(noise/2, DMin, 8), in range needs pnl ≤ −3 and ≥10 bins, ONE streak across the lower edge, 2 distinct valuations (1 when ≥ 2× the threshold). Noisy pairs are untouched. 31-day replay (`scripts/replay/crash_path_replay.js`, variant H8, 315 closes): +0.47 SOL vs live, 8 better / 5 worse, worst −4.4 pp; every faster rule tested on fresh/thin/volatile pairs LOST (dips recover — JEANPHIL fell through the fast-exit zone then ran to +46 %). Shadow logs `[CRASH_REGIME_SHADOW] would-close`; enforce closes through the crash path (`rule:"crash"`, family crash/rug, urgent). Same commit: the exit-swap guard and slippage cap never hold an URGENT exit's remainder (a $24.25 stop-loss remainder of P(DOOM) was held at 6.7 % impact while it kept dumping), and the socket crash twin skips held positions and logs the pair name (it had armed on held positions 100–185 bins below range and logged "undefined").
- **Keep all history** (2026-09-26, operator): nothing the agent records is deleted any more. `price_ticks` (was 30 d), `position_liquidity_ticks` (was 72 h) and `balance_history` (was a 17,280-row cap) are kept in full (re-enable with `TICK_RETENTION_HOURS` / `LIQUIDITY_TICK_RETENTION_HOURS` in .env); `scripts/db_backup.js` keeps every dump (`PG_BACKUP_KEEP` 0 — the oldest dumps are the only copy of what the old retention deleted). The bounded document stores keep their hot window (their jsonb doc is rewritten on every write) but move overflow into the `history_archive` table (migration 007, `db/history-archive.js`): pool-memory snapshots (`pool-snapshots`), rejected candidates (`rejected-reasons` / `-snaps` / `-pools`), `decision-log`, `error-telemetry`, `lessons-evolutions`, `lessons-auto`. PM2 logrotate already retained 100000 files; agent logs go back to 2026-06-14. Growth ≈ 1.7 GB/30 d of ticks + ~170 MB/72 h of liquidity ticks → ~25 GB/yr against ~177 GB free. The dashboard's first balance-history load is bounded to the newest 20,000 rows (older via `?before=` paging). **Restored the same day** (`scripts/restore_history_from_backups.sh`, insert-only, idempotent, safety dump `meridian-pre-restore-20260926-203634.dump`): price_ticks back to 2026-08-13, balance_history back to 2026-06-16 (gap 06-30 → 07-29 exists in no copy), position_liquidity_ticks back to 09-09, and ~18k document items (pool snapshots, rejected candidates, decision log, error telemetry, lessons) into `history_archive`. **Calm-pair crash regime set to `enforce` in prod on 2026-09-26** (operator; config backup `user-config.json.20260926-203419.crash-regime-enforce`).
- **Fee-rate admission** (screening, **live since 2026-09-26** on the operator's instruction, audit 01 §5 Q2/Q3/Q10): `getTopCandidatesRank` = broad envelope fetch → `applyRankSafetyGates` → `admitByFeeRate(safe)` (tools/screening.js, pure): safety floors stay hard (TVL ≥ 10k rug floor for every row incl. Top Performers, critical warnings / single ownership carried by `condensePool`, holders ≥ 500, mcap 100k–20M, bin-step band, blacklists / cooldowns / drain / maxTvl), the dump rule is what the GMGN-pinned dev score already did (window move ≤ −20 % rejects unless Top Performer), a pool under `minTvl` is a **scout** (`_scoutTier`, executor clamps to `scoutSizeSol`; rejected when `scoutTierEnabled=false`) with no clean-history exemption, no intel bar and no trend gate, the velocity gates apply to every pool, quality is ONE sort on `feeRate24hEq` (max of the windowed fee/TVL scaled to a day and the pool's own 24h figure), full-size ahead of scouts, one pool per base token, top `rankAdmitCount`. Removed with it: `computeAdmissionScore`, `prescoreRankCandidates`, the GMGN dev fetch and safety enrichment on the admission path, `hasCleanPoolHistory`/`[TVL_EXEMPT]`, the Top-Performer trend gate, the steady-lane width hints and both executor waivers (fee floor now falls back to the 24h reading for any pool; velocity gates always apply). `rankMinIntelScore` / `rankSteadyMinIntel` / `scoutMinIntel` / `steadyLane*` / `topPerformers*` keys are inert. Intel is still computed for the signal snapshot and decision records. Funnel line: `funnel[rank]: universe → safety → gates → admitted`. Ran one hour in shadow first (`[ADMISSION_SHADOW]`, 8d5b5ec/d44c121): the visible delta is sub-floor pools admitted as 0.15 SOL scouts and fee-led ordering.
- **Single exit evaluator** (2026-09-26, audit 01 §5 Q4 / Phase 3): every mechanical exit is decided by `updatePnlAndCheckExits` (state.js) in ONE order — young stop → stop loss → trailing TP → take profit → round-trip harvest → pumped far above (`outOfRangeBinsToClose`) → unfilled-ladder cap → OOR below → OOR above (price-stable, `oorAboveStableTicks`) → low yield → surge decay (OFF) → toxic conversion (OFF). `getDeterministicCloseRule` (index.js RULE_1…5) is gone, and with it the duplicated stop/OOR/low-yield predicates that carried different guards on the two paths (the mgmt-cycle stop had no confirmation, its low-yield had no adoption/thin-history guard, and an unstable OOR-above `return null` masked every later rule). The exit object carries `action`, `rule`, `family` (= `exit_family`), `urgent` (fastCloseSkipClaim) and `confirm_ticks` (crash/rug: `crashConfirmTicks`; trailing overshoot: 1; else the poller default), stamped by `finalizeExit`; the crash / in-range-rug fast paths stay in the poller and build the same shape. index.js applies the close-efficiency gate (`applyExitGates`) and the harvest→straddle routing (`buildExitAction`) once for both call sites; `EXIT_LABEL` names the rule in Telegram. Stop loss no longer has a 15 s timer (it only ever competed with the index.js backstop): it fires on the evaluation and is confirmed by the poller's distinct-valuation streak. Verification: `scripts/replay/exit_evaluator_replay.js` replays exported ticks through the evaluator with a frozen clock and compares first fires with recorded closes (AUDIT-01 §11 has the export commands and the 7-day result).
- **Grace end re-bases the trailing reference** (2026-09-26): a peak confirmed while the adopted/straddle profit grace suppressed the profit rules is not an actionable trailing reference — SWARM-SOL peaked +2.63 % inside the grace, sat at −5.40 % when the grace ended, and trailing armed against the stale peak and closed it at −5.02 % under the name "trailing TP". At the first tick after the grace (`profit_grace_active` flips), `peak_pnl_pct` is set to the current valuation, pending peaks cleared and `trailing_active` reset (`[GRACE_END]` log), so trailing only measures drops from a peak it was allowed to act on. Stop loss and the fast paths are unaffected.
- **Harvest → straddle** (management, 2026-09-25, operator technique; `harvest-straddle.js`): `harvestStraddleMode` ("off"|"shadow"|"enforce", default **shadow**), `harvestStraddleShape` (spot|curve, spot), `harvestStraddleBins` (34 → ±34, 69 total), `harvestStraddleRatio` (0.5 of the proceeds to base), `harvestStraddleTrendCandles` (3) / `harvestStraddleTrendTimeframe` (5m), `harvestStraddleMinProceedsSol` (0.3), `harvestStraddleMaxImpactPct` (3). At a confirmed ROUND_TRIP_HARVEST (position all-SOL above its range) on a pool whose last 3×5m candles still trend up (`isRebalanceTrendIncreasing`), instead of cashing out the bot does what the operator does on Meteora's Rebalance: `rebalance_position` with `straddle=true` closes the leg, buys the base side with half the proceeds (Jupiter, impact-capped against the active-bin price, fail-closed) and deposits a two-sided ±34-bin range around the price. Proceeds-only sizing, executor pool re-validation and `rebalanceMaxCount` chain depth apply; refused before the close → normal close; aborted after the close → the leg is cash (`markPositionClosedAfterFailedRebalance`, bought base unwound) and the external-close reconciliation writes the record. New position carries `lane:"straddle"` and `rebalance_count+1`; the closed leg gets a `rebalance_leg` perf record. **In place by default** (`harvestStraddleInPlace` true, 2026-09-25): `straddlePositionInPlace` (tools/dlmm.js) uses the DLMM program's `RebalanceLiquidity` instruction on the SAME position account — the instruction Meteora's own Rebalance UI uses (verified on the operator's SWARM-SOL rebalance: InitializePosition/AddLiquidityByStrategy2 then RebalanceLiquidity on the same account) — in three steps: A) SDK `simulateRebalancePositionWithBalancedStrategy(…, yWithdrawBps=ratio)` + `rebalancePosition()` withdraws the ratio of SOL to the wallet and re-centres the rest on the active bin at the same width (the SDK resizes the account's bin range in the same ix); B) Jupiter buys the base side with the withdrawn SOL, impact-capped against the active-bin price, fail-closed; C) a second RebalanceLiquidity tops up the base and re-deposits everything centred. Our state keeps one row (`recordInPlaceStraddle`: range/strategy/amount_x updated, `straddle_count`+1, `lane:"straddle"`, trailing/peak/harvest state reset, `profit_grace_until` = now + `harvestStraddleGraceMinutes` (60) so the fresh range earns before any profit rule looks again), Meteora's PnL stays one lifecycle. Failures leave the account intact: after A the position is narrower/one-sided with the SOL in the wallet, after B the bought base is sold back (best effort). `straddle_count` counts toward `rebalanceMaxCount`. `harvestStraddleInPlace=false` falls back to the close-and-reopen variant. Distinct from the removed roll-up engine (single-sided SOL ladder below the new price). Shadow logs `[STRADDLE_SHADOW] would straddle …`; enforce logs `[STRADDLE]`. ⚠️ The operator's own two-sided positions cannot be graded from the ledger (entry base ratio uncaptured for 290/335 adopted rows) — enable on the operator's judgement, then grade the `lane="straddle"` records.
- **Phase 2 replay verdicts (2026-09-25, audit 01 §10, `scripts/replay/tick_exit_replay.js`):** the proposed peak-crash and slow-bleed exits were replayed over 30 days of 5 s ticks (380 scored closes) against the *current* stack (trailing 2/1.5 confirmed peak + stop −15 + adopted grace). Peak-crash as specified fires 4× in 30 d (0× on bot deploys ahead of trailing); its best variant adds +0.21 SOL/30 d with more truncations than saves — **not built**. Slow-bleed fires 2× (both on positions that recovered) and reaches none of the 37 disasters, which are fast (< 7 h) — **not built**. Tighter stops (−6…−12) are net negative — −15 stays. The current static trailing is +0.92 SOL/30 d better than the adaptive-trailing era on bot deploys and −0.98 on adopted (the operator holds through dips the rules cut). Applied instead: `fastCloseSkipClaim` **ON** (default true), `closeEffQuoteMinIntervalSec` **600**, a 429-aware auto-swap retry (`autoSwapRateLimitExtraAttempts` 2; `[SWAP_RATE_LIMIT]` log — Jupiter's gateway 429'd 103 auto-swaps in 30 d and once left a remainder unsold), and the hold-cohort give-back alert below.
- **Hold-cohort give-back alert** (management, 2026-09-25, audit 01 §4.3): `holdGiveBackAlertPp` (10; 0 = off). Hold mode stays the operator's decision and no exit rule reads it, but when a held position has given back another 10 pp from its confirmed `peak_pnl_pct` the management cycle sends one Telegram line per step (`evaluateHoldGiveBack`/`noteHoldGiveBackAlert` in state.js, latch fields `hold_giveback_alert_pp`/`_at`, reset once the give-back recovers under the first step) and logs `[HOLD_GIVEBACK]`; the briefing marks held positions `🧊 held (peak → −N pp)` and totals the cohort's unrealised give-back. Background: 54 disasters since 08-22, 34 adopted/operator, average peak +55 % → close −17 % on the 28 give-back winners.
