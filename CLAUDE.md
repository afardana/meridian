...
- **Admission shadow** (screening, 2026-09-26, audit 01 §5 Q2/Q3/Q10 — Phase 3 in shadow): `admissionShadowEnabled` (true). `admitByFeeRate(safe, ctx)` (tools/screening.js, pure) is the proposed admission: the safety floors stay hard gates (critical warnings / single ownership carried by `condensePool` from the raw feed, holders ≥ 500, mcap 100k–20M, bin step band, blacklists/cooldowns/drain/maxTvl from `applyRankSafetyGates`), the dump guard is the rule it already is under the GMGN ban (window move ≤ −20 % rejects unless Top Performer), a pool under `minTvl` is a **scout** (or rejected when `scoutTierEnabled=false`) with no clean-history exemption and no intel bar, the velocity gates apply to every pool (no steady-lane waiver), and quality is ONE sort on `feeRate24hEq` (max of the windowed fee/TVL scaled to a day and the pool's own 24h figure — the log-mode Yield input), full-size ahead of scouts, top `rankAdmitCount`. Every rank cycle logs `[ADMISSION_SHADOW] old=[…] new=[…] overlap=… new_only=…[old reason] old_only=…[new reason]`. **Nothing changes until the cut-over**, which the operator must approve on the shadow evidence: it can admit pools the intel bar rejects today, size clean-history sub-floor pools as scouts, admit sub-floor Top Performers without the GeckoTerminal trend gate, and stop admitting steady-envelope pools that only passed via the waivers.
