...
## Current Meridian Operational Context

*   **Bot branch:** Work on the `experimental` branch in both the local checkout (`/Users/Angga/Repos/meridian`) and the live VM checkout (`/opt/meridian` on `oraclevm.fardana.com`). Push bot changes to `origin/experimental`, then fast-forward the VM with `git pull --ff-only origin experimental`. Do not switch the bot to `master`.
*   **Dashboard branch:** The separate dashboard repository remains on `master`; its long-cache JavaScript and CSS assets require matching query-version bumps in `public/index.html`.
*   **Live runtime:** The bot is PM2 app `meridian`; the dashboard is `meridian-dashboard`. Restart only the affected app after a change. Production persistence is PostgreSQL (`PERSIST_BACKEND=pg`, database `meridian`), while `user-config.json` and `.env` remain file-based runtime configuration. `envcrypt.js` loads `.env` with override enabled, so `.env` wins over inherited PM2 or shell values.
*   **LLM provider:** The active OpenAI-compatible endpoint is Ollama cloud (`https://ollama.com/v1`) with `glm-5.3-flash`, authenticated by `OLLAMA_API_KEY`. Ollama uses `reasoning_effort=low` by default in `agent.js` so long tool-calling prompts do not end as reasoning-only empty responses.
*   **Operator HOLD:** `hold_mode=true` is persisted per position. It is surfaced as `On Hold` in dashboard Active Positions and Telegram messages. HOLD suppresses automatic TP, SL, trailing, OOR, crash/rug, flip, health-review, and LLM-driven closes; the final `executeTool()`/DLMM guards enforce this even when a caller bypasses the management loop. Fee claims remain allowed. Only explicit manual `/close <n>`, `/closeall`, or CLI close calls pass the operator override. The hourly health check is read-only. On-chain reconciliation may still mark a position closed when its account is absent because that is truth synchronization, not an exit action.
*   **Manual/multi-asset positions:** `tools/pnl.js` must value both pool legs from their actual mints/prices; token Y must never default to SOL. Adopted positions persist an `asset_profile` and stay observe-only until `postAdoptionValidTicks` consecutive valid samples (default `2`) arm automatic management. Missing asset metadata/prices/deposit basis or extreme PnL divergence marks the sample unsafe and suppresses all automatic exits. `compound_fees` is limited to SOL-quote positions; use `claim_fees` for non-SOL quote assets.
*   **Range-harvest canary:** Pools listed in root `user-config.json` key `rangeHarvestPools` assign open positions the persistent `range_harvest` profile. It suppresses only ordinary profit exits (`takeProfitPct`, trailing TP, profit ratchet), while stop-loss, young-token, crash/rug, OOR, low-yield, and fully-converted round-trip harvest exits remain active. Adopted position strategy is conservatively inferred from normalized on-chain bin shape (`spot`/`curve`/`bid_ask`); external bin-range changes increment `rebalance_count`. The current production canary is ZCAT-SOL only.
*   **Telegram position numbering:** Management Cycle rows are numbered from the same `getMyPositions()` ordering used by `/close <n>`. Keep this ordering aligned when changing either formatter. Avoid redundant instructional footers; `/positions` remains the command for the canonical numbered list.
