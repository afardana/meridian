...
*   **Operator HOLD:** `hold_mode=true` is persisted per position. It is surfaced as `On Hold` in dashboard Active Positions and Telegram messages. HOLD suppresses automatic TP, SL, trailing, OOR, crash/rug, flip, health-review, and LLM-driven closes; fee claims remain allowed. Explicit manual `/close <n>` remains allowed. On-chain reconciliation may still mark a position closed when its account is absent because that is truth synchronization, not an exit action.
