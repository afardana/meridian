...
// Label prefixes that are exit-intent (closes). Deploys and standalone fee claims
// stay "normal" — derived from the label rather than a new options param so none
// of the existing call sites need to change; the label already encodes intent
// ("close:...", "deploy:...", "claim:...").
const EXIT_URGENCY_LABEL_PREFIXES = ["close:"];
  ...
  try {
    if (!existing) {
      await recordPerformance({
        ...
        management_profile: tracked.management_profile || null,
...
export async function rebalancePosition({
  ...
  try {
    ...
    return {
      ...
      bin_range: { min: minBinId, max: maxBinId, active: activeBin.binId, bins_below: bBelow, bins_above: bAbove },
      strategy: target_strategy,
      rebalance_count: newPosRecord?.rebalance_count ?? 1,
      amount_sol: quoteAmount,
      amount_x: tokenXAmount,
      fees_claimed_sol: Number(tracked?.total_fees_claimed_sol ?? 0),
      cumulative_fees_claimed_sol: Number(newPosRecord?.cumulative_fees_claimed_sol ?? 0),
      reason,
      txs: txHashes,
      gas_cost_sol: rebalance_gas_sol,
    };
