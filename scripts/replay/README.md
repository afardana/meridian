...
Primes the persistence caches exactly like `cli.js` (so it works under
`PERSIST_BACKEND=pg`), then joins:
...
- **per-position pool snapshot series** (`pool-memory.getPoolSnapshots(pool)`,
  filtered to the position's own `position` field) — the ~3–10 min-cadence
  time series of `pnl_pct` / `active_bin` / `lower_bin` / `in_range` / …

…into a normalized `scripts/replay/dataset.json`. `--summary` prints coverage
(n positions, n with a snapshot series, n with path features, n with post-close
probes, n dense-enough for the crash rule, date range).
...
|--------|-------------|----------|-------------------------|
| `oor` (OOR-below wait) | `state.js updatePnlAndCheckExits` OOR-below branch | `outOfRangeWaitMinutesBelow` ∈ {15,30,45,63,90} | Fires at the first snapshot that is OOR-below (`active_bin < lower_bin`) and has been continuously so for ≥ limit minutes. Exit PnL = that snapshot's mark. |
